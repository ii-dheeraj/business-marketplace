...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (sellerOrderError) {
      ...
      return NextResponse.json({ error: "Failed to fetch seller order", details: sellerOrderError.message }, { status: 500 })
    }
    
    if (!sellerOrder) {
      console.error('[SELLER ORDER DETAILS API ERROR] Seller order not found for orderId:', orderId, 'sellerId:', sellerId)
      return NextResponse.json({ error: "Seller order not found" }, { status: 404 })
    }
